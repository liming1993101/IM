...
import socekt.lm.socektdemo.utils.DialogFactory;
...
public class LoginActivity extends MyActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mResiterBt= (Button) findViewById(R.id.register);
        ...
        mResiterBt.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
    ...
    protected void onResume() {// 在onResume方法里面先判断网络是否可用，再启动服务,这样在打开网络连接之后返回当前Activity时，会重新启动服务联网，
        ...
        if (new NetUtil().isNetworkAvailable(LoginActivity.this)) {
    ...
    private void userLogin() {
        ...
        if (myApplication.isClientStart())
        {
            ...
            User u = new User();
            u.setId(Integer.parseInt(mEmailView.getText().toString()));
            u.setPassword(Encode.getEncode("MD5",mPasswordView.getText().toString()));
            o.setObject(u);
            out.setMsg(o);
        }
        else
        {
            Toast.makeText(LoginActivity.this,"服务器抽风了！",Toast.LENGTH_SHORT);
        }
    }
            ...
            switch (msg.getType())
            {
                case LOGIN:
                    ...
                    if (list.size()>0)
                    {
                        ...
                        util.setId(mEmailView.getText().toString());
                        util.setPasswd(mPasswordView.getText().toString());
                    ...
                    {
                        Toast.makeText(LoginActivity.this,"你的账号或者密码错误!",Toast.LENGTH_SHORT).show();
                    }
